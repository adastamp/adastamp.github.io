...
```haskell
>>> BSL.putStrLn $ encodePretty (fromJust $ decode $ fromJust $ rb ^? responseBody :: Value)
{
    "appVersionInfo": {
...
>>> let opts = defaults & header "Authorization" .~ ["Bearer " <> (Char.pack key)]
>>> rb <- getWith opts $ host <> "/api/v1/static/last/block"
>>> Char.putStrLn $ encodePretty (fromJust $ decode $ fromJust $ rb ^? responseBody :: Value)
3922811
```
